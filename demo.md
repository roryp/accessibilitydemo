...
1. Open **`accessibility-issues-demo.html`** in Chrome/Edge (serve locally or file:// protocol)
...
1. Open **`accessibility-issues-demo.html`** flagged by FastPass/Axe Linter
...
1. Open **Copilot Chat** (Ctrl+Alt+I or via the sidebar icon)
2. Prompt:
   ...
   keyboard accessibility, and any other common accessibility problems. Make the page fully 
   accessible for screen readers and users with disabilities.
   ```

3. Review the file diff Copilot proposes
...
|---------------|----------|--------------|
| `accessibility-check.yml` | Accessibility Check | Launches a local server, runs `run-axe-tests.js` (axe-core) on **`accessibility-issues-demo.html`**, generates an HTML report, and fails if violations remain |
...
3. **Body:** "Ensure full operability without a pointing device per WCAG 2.1 AA standards"
4. Assign the issue to **@github-copilot**

The coding agent will:
- Create a branch
- Commit fixes to `accessibility-issues-demo.html`
...
- **Problem file:** `accessibility-issues-demo.html` (contains deliberate accessibility violations)
- **Solution file:** `accessibility-fixed-demo.html` (shows the corrected version)
- **Test runners:** `run-axe-tests.js`, `ai_accessibility_analyzer.py`